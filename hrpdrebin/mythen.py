...
def find_mythen_files(
    ...
    visit=None,
    year=None,
    bl_dir=DEFAULT_BL_DIR,
    ending=("mac[-_][0-9]*.dat", "mythen[-_][0-9]*.dat"),
):
...
def parse_metadata(f):
    ...
    for k in d.keys():
        if k != "metadata":
            for v in d[k]:
                match = mythen_reg.match(str(v))
                if match:
                    mythen_files.append(path.join(dir, match.group(0)))
...
def preserve_filesystem(dpath, output):
    ...
    def split_at_visit(
        dpath,
    ):  # Split at the visit directory by finding the first /data/ directory
        spath = dpath.split("/")
        try:
            di = spath.index("data")
        except ValueError:
            print(
                "Cannot save the data into a directory data/year/visit/processed/ if input is not under data/year/visit/"
            )
            raise
        br = di + 3
...
def process_and_save(
    ...
    print(
        "angle: {}, delta: {}, rebinned: {}, summed: {}, files: {}, output: {}, weights: {}, ext: {}".format(
    ...
    if summed and (progress is None or not progress.wasCanceled()):
        ...
        if not output or not path.split(output)[1]:
            fbase = set(fbases)
            if len(fbase) == 1:  # If all the prefixes are identical just use it
                prefix = fbase.pop()
            else:
                prefix = "_".join([path.splitext(path.split(f)[1])[0] for f in files])
            if ext is None:
                ext = path.splitext(files[0])[
                    1
                ]  # Use the filename extension of the first file
...
def parse_range_list(lst):
    ...
    """
    ranges = [p.split("-") for p in lst.split(",")]
    values = []
    import math
